...
def setup(region, non_interactive):
    ...
    console.print(f"\n[yellow]Fetching available Claude models from {region}...[/yellow]")
    ...
    if non_interactive and models:
        ...
        console.print(f"[yellow]Using first available model: {selected_model['name']}[/yellow]")
    else:
...
def status():
    ...
    if not settings:
        ...
        console.print("Run 'claude-setup setup' to configure Claude for AWS Bedrock.")
        return
    
    console.print(Panel.fit(
        Text("Claude Bedrock Configuration", style="bold blue")
    ))
    ...
    console.print(f"\n[dim]Settings file: {config_manager.settings_path}[/dim]")
