...
class BedrockClient:
    ...
    def list_claude_models(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            cmd = ['aws', 'bedrock', 'list-inference-profiles', '--region', self.region]
            ...
            if 'AccessDeniedException' in e.stderr:
                raise Exception("Access denied. Please check your AWS permissions for Amazon Bedrock.")
            elif 'not authorized' in e.stderr:
                raise Exception("Not authenticated with AWS. Please run 'aws configure' or set up your AWS credentials.")
            else:
