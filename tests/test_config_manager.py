...
class TestConfigManager:
    ...
    def test_save_settings_new_file(self, mock_load_settings, mock_file, mock_mkdir):
        ...
        # Check that json.dump was called - the actual data is written by json.dump
        # json.dump calls write multiple times, so we just verify write was called
        assert mock_file().write.called

    @patch('claude_setup.config_manager.Path.mkdir')
    @patch('builtins.open', new_callable=mock_open)
    @patch('claude_setup.config_manager.ConfigManager.load_settings')
    def test_save_settings_update_existing(self, mock_load_settings, mock_file, mock_mkdir):
        """Test saving settings to update existing file."""
        # Arrange
        config_manager = ConfigManager()
        existing_settings = {"EXISTING_KEY": "existing_value", "CLAUDE_CODE_USE_BEDROCK": "0"}
        new_settings = {"CLAUDE_CODE_USE_BEDROCK": "1", "AWS_REGION": "us-west-2"}
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('claude_setup.config_manager.ConfigManager.load_settings')
    def test_save_settings_special_characters(self, mock_load_settings, mock_file, mock_mkdir):
    ...
    @patch('claude_setup.config_manager.Path.mkdir', side_effect=PermissionError("Permission denied"))
    ...
    @patch('claude_setup.config_manager.ConfigManager.load_settings')
    def test_save_settings_file_permission_error(self, mock_load_settings, mock_file, mock_mkdir):
    ...
    @patch('claude_setup.config_manager.Path.unlink', side_effect=PermissionError("Permission denied"))
    @patch('claude_setup.config_manager.Path.exists')
